 ...
 */
export async function approveToken(web3: Web3, erc20Address: string, sender: string, approvedSpender: string) {
  const erc20 = new web3.eth.Contract(ERC20Abi as any, erc20Address)

  await erc20.methods.approve(approvedSpender, BigNumber.from("2").pow(256).sub(1)).send({from: sender})
}
...
export async function getApprovedQuantity(
  ...
  const erc20 = new web3.eth.Contract(ERC20Abi as any, erc20Address)
  let allowance = BigNumber.from(await erc20.methods.allowance(sender, approvedSpender).call())
  return allowance as BigNumber
}

export async function getBalance(web3: Web3, erc20Address: string, address: string): Promise<BigNumber> {
  const erc20 = new web3.eth.Contract(ERC20Abi as any, erc20Address)
