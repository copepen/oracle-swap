...
export function OrderEntry(props: {
  ...
  const [qtyBn, setQtyBn] = useState<BigNumber | undefined>(BigNumber.from("1"))
  const [authorizedQty, setAuthorizedQty] = useState<BigNumber>(BigNumber.from("0"))
  const [isAuthorized, setIsAuthorized] = useState<boolean>(false)

  const [spentToken, setSpentToken] = useState<TokenConfig>(props.baseToken)
  const [approxQuoteSize, setApproxQuoteSize] = useState<number | undefined>(undefined)
  ...
  useEffect(() => {
    async function helper() {
      if (props.web3 !== undefined && props.account !== null) {
        setAuthorizedQty(
          await getApprovedQuantity(props.web3!, spentToken.erc20Address, props.account!, props.swapContractAddress)
        )
      } else {
  ...
  useEffect(() => {
    if (qtyBn !== undefined && props.approxPrice !== undefined) {
      setApproxQuoteSize(tokenQtyToNumber(qtyBn, props.baseToken.decimals) * props.approxPrice)
    } else {
    ...
    <div>
      <div>
        ...
        {qtyBn !== undefined && approxQuoteSize !== undefined ? (
          props.isBuy ? (
            <p>
              Pay {approxQuoteSize.toFixed(3)} {props.quoteToken.name} to receive{" "}
              {tokenQtyToNumber(qtyBn, props.baseToken.decimals).toFixed(3)} {props.baseToken.name}
            </p>
          ) : (
            <p>
              Pay {tokenQtyToNumber(qtyBn, props.baseToken.decimals).toFixed(3)} {props.baseToken.name} to receive{" "}
      ...
      <div className={"swap-steps"}>
        {props.account === null || props.web3 === undefined ? (
          ...
          <div>
            ...
            <button
              onClick={async () => {
                await approveToken(props.web3!, spentToken.erc20Address, props.account!, props.swapContractAddress)
              }}
...
async function sendSwapTx(
  ...
  await swapContract.methods.swap(isBuy, qtyWei, priceFeedUpdateData).send({value: updateFee, from: sender})
}
