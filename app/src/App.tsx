...
function App() {
  ...
  const [pythOffChainPrice, setPythOffChainPrice] = useState<Record<HexString, Price>>({})
  ...
  useEffect(() => {
    ...
    if (basePrice !== undefined && quotePrice !== undefined) {
      const exchangeRate = basePrice.getPriceAsNumberUnchecked() / quotePrice.getPriceAsNumberUnchecked()
      const lastUpdatedTime = new Date(Math.max(basePrice.publishTime, quotePrice.publishTime) * 1000)
      ...
      <div className="control-panel">
        ...
        <div>
          ...
          {chainState !== undefined ? (
            <div>
              <p>
                {tokenQtyToNumber(chainState.accountBaseBalance, CONFIG.baseToken.decimals)} {CONFIG.baseToken.name}
                <MintButton
                  web3={web3!}
                  sender={account!}
              ...
              <p>
                {tokenQtyToNumber(chainState.accountQuoteBalance, CONFIG.quoteToken.decimals)} {CONFIG.quoteToken.name}
                <MintButton
                  web3={web3!}
                  sender={account!}
